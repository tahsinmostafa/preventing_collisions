...
class EnvViewer(object):
    ...
    def __init__(self, env: 'AbstractEnv', config: Optional[dict] = None) -> None:
        ...
        pygame.draw.line(self.sim_surface, start_pos= self.sim_surface.centering_position, end_pos= self.sim_surface.centering_position, color="yellow")
        ...
        self.frame = 0
        self.directory =  "C:/Users/mosta/highway_env_saved_frames"     # None
    ...
    def display(self) -> None:
        ...
        #if self.SAVE_IMAGES and self.directory and self.env.time % (self.config['simulation_frequency'] / self.config['policy_frequency']) == 0:    # added the last condition here
        if self.SAVE_IMAGES and self.directory:    
            # self.env.time counts the number of time steps in an episode; basically the episode length
            #pygame.image.save(self.sim_surface, str(self.directory / "highway-env_{}.png".format(self.frame)))   # this is the original version
            pygame.image.save(self.sim_surface, self.directory + "/" + "highway-env_{}.png".format(self.frame))
            self.frame += 1
