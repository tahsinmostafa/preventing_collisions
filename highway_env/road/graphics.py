...
class RoadGraphics(object):
    ...
    def display_traffic(road: Road, surface: WorldSurface, simulation_frequency: int = 15, offscreen: bool = False) \
        ...
        """
        non_ego_display_tag = False
        perception_distance = 5 * Vehicle.MAX_SPEED  # make sure to match this with env's perception distance constant variable
        ego_v = road.vehicles[0]
        list_of_obs_vehicles = road.close_vehicles_to(ego_v, perception_distance, count=6)   # make sure match count with number of surrounding vehicles; also refer to observation.py, observe() 
        if road.record_history:
            for v in road.vehicles:
                VehicleGraphics.display_history(v, surface, simulation=simulation_frequency, offscreen=offscreen)
        VehicleGraphics.display(ego_v, surface, offscreen=offscreen, is_ego = True)
        for v in road.vehicles[1:]:
            if v in list_of_obs_vehicles:
                non_ego_display_tag = True
            VehicleGraphics.display(v, surface, offscreen=offscreen, non_ego_display_tag = non_ego_display_tag)
            non_ego_display_tag = False
