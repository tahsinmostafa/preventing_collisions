...
class StraightLane(AbstractLane):
    ...
    def heading_at_modified(self, longitudinal: float) -> float:
        chance = random.random() 
        if chance >= 0.3:
            return self.heading - 2*np.pi
        elif chance > 0.3 and chance < 0.7:
            return self.heading - 1.5*np.pi
        else:
            return self.heading - 2.5*np.pi
