...
from highway_env.vehicle.random_flow_vehicle import RandomFlowVehicle
if TYPE_CHECKING:
...
class VehicleGraphics(object):
    ...
    def display(cls, vehicle: Vehicle, surface: "WorldSurface",
        ...
        # this is plotting the top-left white points
        pygame.draw.line(surface, start_pos= pos, end_pos= pos, color="white")
        ...
        if non_ego_display_tag or is_ego:
            label = True
        # Label
        if label:
            ...
            text = "#"
            if is_ego:
                text = str(int(vehicle.crashed))
            text = font.render(text, 1, (10, 10, 10), (255, 255, 255))
            surface.blit(text, list(np.array(position) - 0.2))
    ...
    def get_color(cls, vehicle: Vehicle, transparent: bool = False) -> Tuple[int]:
        ...
        if getattr(vehicle, "color", None):
            ...
            color = cls.EGO_COLOR
        # added this
        elif isinstance(vehicle, RandomFlowVehicle):
            color = cls.EGO_COLOR
        if transparent:
