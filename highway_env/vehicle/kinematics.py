...
class Vehicle(RoadObject):
    ...
    @classmethod
    def create_random_modified(cls, road: Road,
                      speed: float = None,
        ...
        # x0 += offset * road.np_random.uniform(0.9, 1.1)   # uniform(0.9,1.1)

        x0 = random.uniform(10,40)    # lane.start = 0, lane.end = 100  --> coordinate values
       
        y = random.uniform(1.5,2.5)  # need to check if 5 is the right number
                                 # should be relative to number of lanes
        #y = 1
        if random.random() >= 0.2:
            #x0 = lane.end[0] / 2 - offset
            head = lane.heading_at_modified(x0)
            v = cls(road, lane.position(x0, y), head, speed)
        else:
            v = cls(road, lane.position(x0, y), lane.heading_at(x0), speed)
        return v
    ...
    def clip_actions(self) -> None:
        if self.crashed:
            ...
            # added this in hope that vehicles will stand still right after collision
            #self.speed = 0
